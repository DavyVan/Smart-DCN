...
int main()
{
	...
	for (int i = 0; i < rec_count; i++)
	{
		...
		// client side
		clientside[records[i].src] << "iperf3 -p " << records[i].port << " -f m -i 0 -J -c " << IPs[records[i].dst] << " -V -n " << (int)round(records[i].demand * RATE_MULTIPLIER) << "M --logfile ~/result-" << records[i].src << "-" << records[i].dst << "-" << records[i].port << ".txt" << endl;

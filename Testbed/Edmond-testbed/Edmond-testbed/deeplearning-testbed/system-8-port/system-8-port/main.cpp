...
void initialToRLink(int topo[][nodenum])
{
	...
	for (int i = 0; i < num; i++)
	{
		topo[i+ToRnum][(i + 1) % num+ToRnum] = 1;
		topo[(i + 1) % num+ToRnum][i+ToRnum] = 1;
...
int main()
{
	...
	for(int count = 0;count < 1 ;count ++)
	{
		...
		string bestTopoPath = "..\\..\\..\\Sample_data\\edmond_4\\topo\\topo0.txt";
		ifstream in_topo(bestTopoPath.c_str());
		for (int i = 0; i < nodenum; i++)
		{
			for (int j = 0; j < nodenum; j++)
			{
				in_topo >> bestTopo[i][j];
			}
		}
		in_topo.close();
